 ...
 * @LastEditTime: 2019-12-11 09:25:12
...
export default {
  ...
  methods: {
    ...
    ReplaceUrl(text) {
      ...
      // const regExp = /(http(s)?:\/\/)?(www\.)?[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+(:\d+)*(\/\w+\.\w+)*.*?(?=\s)/;
      // const { 0: targetStr, index } = regExp.exec(text) || {};
      // return targetStr
      //   ? text.slice(0, index) +
      //       `< a href=" ">${targetStr}</ a>` +
      //       this.ReplaceUrl(text.slice(index + targetStr.length))
      //   : text;
    },
    clickmore() {
      this.flag = !this.flag;
  ...
  computed: {
    ...mapState(["language", "product_id", "MerchantId"])
  }
};
</script>
