 ...
 * @LastEditTime: 2019-12-11 15:52:51
...
<template>
  <div class="page-wrap">
    ...
    <div class="listNumber">
      ...
      <a v-for="(item,index) in listPdf" :key="index" target="_blank" :href="item" class="loading">
        <img src="../assets/images/confirmationOfCover@3x.png" alt />
        <span>{{listNumber[index]}}</span>
        <span>{{$t('confirm_of_cover')}}下載</span>
...
export default {
  ...
  methods: {
    ...
    // }
    //创建多条a标签
    // async PolicyConfirmation() {
    //   for (var i = 0; i < this.listPdf.length; i++) {
    //     window.open(this.listPdf[i]);
    //   }
    // }
  },
  async mounted() {
    ...
    try {
      ...
      this.listNumber = res.data.awi_policy_no_list;
      this.listPdf = res.data.awi_policy_link;
    } catch (error) {
      if (error.msg) {
        //请求错误
        this.$createToast({
